...
6. Dedicated-worker tests in Chrome and Firefox prove slice-path parity for the PNG Suite grayscale, RGB, palette, and RGBA corpus at 8 or 16 bits, plus a generated Adam7 RGBA input. RGBA, buffered PNG/JPEG, direct writer, and 64 KiB chunk writer outputs are covered.
7. Boundary coverage includes checked seek and EOF behavior, limit rejection before the first content read, exact callback result length and type, Promise rejection, invalid and truncated input diagnostics, and identity-preserving input and output callback exceptions.
8. A 3,147,060-byte high-entropy input required 391 callback reads, reread 49,152 bytes, issued 18 seeks, and never requested more than the existing 8 KiB `BufReader` capacity. One local debug run measured 437 ms seekable versus 343 ms slice in Chrome and 436 ms versus 415 ms in Firefox; WebAssembly linear memory did not grow during the seekable operation. These are single-run observations, not thresholds.
9. The spike is accepted for provisional WebAssembly API integration. No second JavaScript prefetch cache is added because decoder rereads were about 1.6% and the existing buffer already bounds each callback. Installed-package fresh-worker comparison remains required before the Pages worker switches to direct File input.

### Phase 37 decisions

1. `thumbnailPngFromSeekable` and `thumbnailPngFromSeekableToChunks` expose the proven synchronous `readAt(offset, length)` contract without changing the existing slice APIs.
2. The WebAssembly adapter accepts only non-negative JavaScript safe-integer lengths, implements checked Start/Current/End seeks within `0..=inputLength`, requires exact-length `Uint8Array` callback results, and fails closed on Promises, wrong types, and short or long reads.
3. Buffered seekable output reuses the existing PNG, JPEG, and RGBA reader APIs. Chunked seekable output supports PNG and JPEG and reuses the existing 64 KiB callback writer and result metadata.
4. JavaScript values thrown by input or output callbacks are retained and rethrown unchanged. Input callback failure has deterministic precedence over output callback failure, followed by the streamthumb diagnostic.
5. The package remains runtime-neutral: it accepts a numeric length and callback rather than a DOM `File`. Browser documentation places `FileReaderSync` and `Blob.slice()` in a dedicated worker; Node.js and Deno may provide equivalent synchronous range readers.
6. Dedicated-worker Chrome and Firefox tests cover output parity, chunk parity, bounded reads, pre-read input-limit rejection, callback contract failures, and exception identity. The installed-tarball Chrome consumer uses a real structured-cloned `Blob` and `FileReaderSync` worker.
7. Slice and seekable WebAssembly execution share one internal reader enum and common non-inlined buffered and chunked drivers so decoder, encoder, and callback paths are generated once. Fat LTO, one code-generation unit, aborting panics, and the existing `wasm-opt -Oz` keep the pinned Rust 1.85 release package at 536,273 unpacked bytes, below the unchanged 550,000-byte limit.
