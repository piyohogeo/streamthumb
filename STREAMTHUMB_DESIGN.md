...
2. `planThumbnailPngFromSeekable` exposes the same range-reader contract for header-only planning and returns the same plain-object plan as `planThumbnailPng`.
3. The WebAssembly adapter accepts only non-negative JavaScript safe-integer lengths, implements checked Start/Current/End seeks within `0..=inputLength`, requires exact-length `Uint8Array` callback results, and fails closed on Promises, wrong types, and short or long reads.
4. Buffered seekable output reuses the existing PNG, JPEG, and RGBA reader APIs. Chunked seekable output supports PNG and JPEG and reuses the existing 64 KiB callback writer and result metadata.
5. JavaScript values thrown by input or output callbacks are retained and rethrown unchanged. Input callback failure has deterministic precedence over output callback failure, followed by the streamthumb diagnostic.
